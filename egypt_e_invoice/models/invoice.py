...
class AccountMove(models.Model):
    ...
    def get_invoice_dics_cron(self,start_date=False):
        domain = [('type', 'in', ['out_invoice','out_refund','in_refund']),('state','=','posted'),('is_signed','=',False)]
        if start_date:
    ...
    def get_invoice_dic(self):
        ...
        for line in self.invoice_line_ids:
            # for multi currency
            price = round(line.price_unit, 3)
            # sales_total = round(line.quantity * price, 3)
            sales_total =  float('%.5f' % (line.quantity * line.price_unit))
            inv_sales_total += sales_total
            discount_amount = round(line.discount * sales_total / 100, 3)
            inv_discount += discount_amount
            # net_total = sales_total - discount_amount
            net_total = float('%.5f' % (sales_total - discount_amount))
            ...
            for tax_id in line.tax_ids.sorted(key=lambda r: r.sequence):
                current_tax_amount = round(amount_to_tax * tax_id.amount / 100,2)
                taxes.append({
            ...
            totalTaxableFees = round(sum(t['amount'] for t in taxes), 2)
            total = round(net_total + totalTaxableFees, 2)
        ...
        invoice_dic = {
            ...
            # "totalDiscountAmount": inv_discount,
            "totalDiscountAmount": float('%.5f' % inv_discount),
            # "totalSalesAmount": inv_sales_total,
            "totalSalesAmount": float('%.5f' % inv_sales_total),
            # "netAmount": inv_net_total,
            "netAmount": float('%.5f' % inv_net_total),
            "taxTotals": taxTotals,
            # "totalAmount": self.amount_total,
            "totalAmount": float('%.5f' % self.amount_total),
